...
class ExpenseAgent:
    ...
    def extract_from_bytes(self, image_bytes: bytes, media_type: str) -> Dict[str, Any]:
        """Encode l'image, appelle Groq Vision, parse et sécurise le JSON retourné."""
        if not image_bytes:
        ...
        try:
            ...
            # Compatibilité avec certaines versions du SDK qui attendent max_tokens.
    ...
    def _parse_json(self, raw_content: Optional[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Défense supplémentaire si le modèle ajoute accidentellement du texte.
    ...
    def _normalize_result(self, data: Dict[str, Any]) -> Dict[str, Any]:
        normalized: Dict[str, Any] = {field: data.get(field, None) for field in EXPECTED_FIELDS}
        ...
        if isinstance(type_document, str):
            ...
            if type_document == "hotel":
                type_document = "hôtel"
            normalized["type_document"] = type_document if type_document in ALLOWED_TYPES else "autre"
        elif type_document is None:
        ...
        if isinstance(confiance, str):
            ...
            normalized["confiance"] = confiance if confiance in ALLOWED_CONFIDENCE else "basse"
        else:
            normalized["confiance"] = "basse"

        normalized["montant_ttc"] = self._to_float_or_none(normalized.get("montant_ttc"))
        normalized["tva"] = self._to_float_or_none(normalized.get("tva"))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Tester l'extraction d'une note de frais en CLI")
    parser.add_argument("image_path", help="Chemin vers une image de ticket/facture")
    ...
    agent = ExpenseAgent()
    result = agent.extract_from_bytes(image_path.read_bytes(), media_type)
